...
import { getLaundryComplaints } from "../../../axios/complaints";
...
const DIRECTION_TABS = [
  ...
  { value: "sent", label: "Sent to Business" },
  { value: "received", label: "Received from Business" },
];
...
const LaundryComplaintsSection = () => {
  ...
  // Fetch linked businesses list for the filter dropdown
  useEffect(() => {
    let mounted = true;
    const fetchBusinesses = async () => {
      try {
        const res = await getLaundryTenants({ limit: 100 });
        const list =
          res?.data?.data?.items ||
          res?.data?.items ||
          res?.data?.data ||
        ...
        if (mounted && Array.isArray(list)) {
          ...
          setBusinessesList(formatted);
        }
      } catch (err) {
        console.error("Failed to load linked businesses for filter:", err);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchComplaints = async () => {
      setIsLoading(true);
      try {
        const res = await getLaundryComplaints({
          page: currentPage,
        ...
        if (mounted) {
          console.error("Failed to fetch laundry complaints:", err);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        label: "Direction",
        render: (_, row) => {
          const isSent = row.senderType === "laundry";
          return (
      ...
      },
      {
        ...
        render: (_, row) => (
          <Button
            ...
            onClick={() => navigate(`/laundry/complaints/${row.id}`)}
          >
            View
          </Button>
        ),
        ...
        <div className="space-y-4 p-4">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(220px,1fr)_180px_160px_160px_auto]">
            ...
            <Dropdown
              ...
              search={businessOptions.length > 6}
            />
            <Dropdown
              onChange={(val) => {
                setStatusFilter(val);
          ...
          <Table
            ...
            emptyText={
              direction === "sent"
                ? "No outbound complaints submitted to businesses yet."
                : direction === "received"
                  ? "No inbound complaints received from businesses."
...
export default LaundryComplaintsSection;
