...
import { Search, MapPin, Filter, AlertTriangle, Radio, Smartphone, MoreHorizontal } from "lucide-react";
import {
    scannersData,
    scannerTypeOptions,
    scannerModeOptions,
...
const ScannerTable = () => {
    ...
    const filteredScanners = useMemo(() => {
        return scannersData.filter((scanner) => {
            ...
            return matchesSearch && matchesType && matchesMode && matchesLocation && matchesStatus;
        });
    ...
    const columns = [
            ...
            render: (_, row) => (
                <div className="flex min-w-0 items-center gap-3">
                    <IconWrapper
                        ...
                        variant={row.status === "Inactive" ? "neutral" : row.status === "Warning" ? "warning" : "purple"}
        ...
        },
        {
            ...
            render: (_, row) => (
                <Badge
                    className=""
                    leftIcon={row.type === "Fixed" ? <Radio size={12} /> : <Smartphone size={12} />}
                    size="sm"
        ...
        },
        {
            ...
            sortable: true,
            render: (_, row) => (
                <Badge variant={row.mode === "Entry" ? "info" : row.mode === "Exit" ? "success" : "neutral"} size="sm" className="bg-opacity-10 text-(--theme-text-primary)">
                    {row.mode}
                </Badge>
            ),
        },
        {
            ...
            sortable: true,
            render: (_, row) => (
                <span className={`text-sm font-medium ${row.location === "Roaming" ? "italic text-(--theme-text-muted)" : "text-(--theme-text-secondary)"}`}>
                    {row.location}
                </span>
            ),
        },
        {
            ...
            sortable: true,
            render: (_, row) => (
                <span className={`text-sm font-medium ${row.operator ? "text-(--theme-text-secondary)" : "text-(--color-overdue) flex items-center gap-1.5"}`}>
        ...
        <Card padding="0" rounded="18px">
            <div className="grid gap-3 px-4 py-4 lg:flex lg:flex-wrap lg:items-center">
                ...
                <div className="w-full lg:w-[180px]">
                    <Dropdown
                        leftIcon={<Filter size={14} className="text-(--theme-text-muted)" />}
                        onChange={(val) => { setTypeFilter(val); resetCurrentPage(); }}
                ...
                <div className="w-full lg:w-[180px]">
                    <Dropdown
                        onChange={(val) => { setModeFilter(val); resetCurrentPage(); }}
                ...
                <div className="w-full lg:w-[240px]">
                    <Dropdown
                        ...
                        onChange={(val) => { setLocationFilter(val); resetCurrentPage(); }}
