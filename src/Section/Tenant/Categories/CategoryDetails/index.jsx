...
const normalizeCategoryDetails = (response = {}) => {
  ...
  const rawUsage = category.usage ?? category.usageStatus;
  const totalAssets = Number(
    overview.totalMappedAssets ?? category.totalMappedAssets ?? 0,
  );
  const isUsed = rawUsage
    ? String(rawUsage).toLowerCase() === "in use"
    : totalAssets > 0;

  return {
    code: category.categoryCode || category.code || "—",
    title:
      category.title || category.translations?.en?.title || "Unnamed Category",
    description:
    ...
    usageVariant: isUsed ? "info" : "neutral",
    created: formatDateTime(overview.createdAt || category.createdAt),
    updated: formatDateTime(overview.lastUpdatedAt || category.updatedAt),
    overview: {
      totalMappedAssets:
        Number(overview.totalMappedAssets ?? category.totalMappedAssets ?? 0) ||
        0,
    ...
    recentAssets: Array.isArray(category.recentAssets)
      ? category.recentAssets.map((asset) => {
          ...
          const laundry =
            asset.assignedLaundry || asset.assignedLaundryLink?.laundry || {};
          return {
            ...
            laundry:
              ...
              "—",
            laundryContact:
              laundry.contactPersonName || laundry.fullName || "—",
            status: asset.statusLabel || formatStatusLabel(asset.status),
            statusVariant: STATUS_VARIANTS[asset.status] || "neutral",
            assetWashCount:
              Number(asset.assetWashCount ?? asset.washCount ?? 0) || 0,
            tagWashCount:
              Number(asset.tagWashCount ?? tag.totalLaundryCycles ?? 0) || 0,
            ...
            mappingStatusVariant:
              STATUS_VARIANTS[tag.mappingStatus] || "neutral",
            linkedDate: formatDateWithUserPreferences(
              asset.linkedDate || tag.linkedDate || tag.linkedAt,
            ),
...
const CategoryDetails = () => {
  ...
  useEffect(() => {
    ...
    getTenantCategoryDetails(id)
      ...
      .catch((error) => {
        ...
        setCategory(null);
        setLoadError(
          getApiErrorMessage(error, "Unable to load category details"),
        );
      })
      .finally(() => {
  ...
  const summaryItems = [
    {
      label: "Total Mapped Assets",
      value: category.overview.totalMappedAssets,
      icon: Cuboid,
      accent: true,
    },
    {
      label: "Wash Limit",
      value: category.overview.washLimit,
      icon: RefreshCcw,
    },
    {
      label: "Created",
  ...
  const columns = [
    {
      ...
      render: (_, row) => (
        <div className="min-w-48">
          <div className="font-black text-(--theme-text-primary)">
            {row.assetName}
          </div>
          <div className="mt-1 font-mono text-xs font-semibold text-(--theme-text-muted)">
            {row.assetCode}
          </div>
      ...
      render: (_, row) => (
        <div className="min-w-64 space-y-1.5">
          ...
          <div className="flex flex-wrap gap-1.5">
            <Badge size="sm" variant={row.tagStatusVariant}>
              {row.tagStatus}
            </Badge>
            <Badge size="sm" variant={row.mappingStatusVariant}>
              {row.mappingStatus}
            </Badge>
          </div>
        </div>
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="min-w-36">
          <div className="font-black text-(--theme-text-primary)">
            Asset {row.assetWashCount}
            {row.washLimit != null ? ` / ${row.washLimit}` : ""}
          </div>
          <div
            className={`mt-1 text-xs font-semibold ${row.washCountMatches ? "text-(--badge-ready-text)" : "text-(--color-overdue)"}`}
          >
            Tag {row.tagWashCount} ·{" "}
            {row.washCountMatches ? "Matched" : "Mismatch"}
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex min-w-40 justify-end gap-1.5">
          ...
          <Button
            disabled={!row.tagId}
            leftIcon={<TagIcon size={13} />}
            size="sm"
            variant="outline"
            onClick={() => navigate(`/business/tags/${row.tagId}`)}
          >
            Tag
          </Button>
        </div>
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="m-0 mt-1 text-sm font-semibold text-(--theme-text-muted)">
          Category overview, asset usage and mapped RFID tags.
        </p>
      </div>
      ...
      <Card padding="18px 22px" rounded="18px">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="font-mono text-xs font-bold text-(--theme-text-muted)">
                  {category.code}
                </span>
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            <Badge
              size="sm"
              variant={STATUS_VARIANTS[category.status] || "neutral"}
            >
            ...
            <Badge
              size="sm"
              variant={
                category.usageVariant ||
                (category.usageState === "used" ? "info" : "neutral")
              }
            >
              {category.usage}
            </Badge>
      ...
      <Card padding="18px" rounded="18px">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <h3 className="m-0 text-lg font-black text-(--theme-text-primary)">
              Assets
            </h3>
            <p className="m-0 mt-1 text-xs font-semibold text-(--theme-text-muted)">
              Assets currently mapped to this category.
            </p>
          </div>
          <Badge variant="info" size="sm">
            {category.recentAssets.length}
          </Badge>
        </div>
        <Table
          columns={columns}
