    ...
    },
    {
      ...
      render: (_, row) => (
        <Badge
          size="sm"
          variant="neutral"
          className="bg-(--theme-surface-hover) border-none text-(--theme-text-secondary)"
        >
          📁 {row.category}
        </Badge>
      ),
    ...
    <div className="px-4 pb-4 mt-4">
          ...
          {undoState === "timer" && (
            <>
              <span className="text-sm font-bold text-(--theme-text-secondary)">
                Auto {isCheckIn ? "Check-In" : "Check Out"} In{" "}
                <span className="text-(--color-fresh-mint)">02:00</span>
              </span>
        ...
        <Alert variant="warning" className="mb-4">
          <div className="flex justify-between items-center w-full">
            <span className="text-sm font-bold text-[color-mix(in_srgb,var(--color-sunlit-gold)_70%,var(--theme-text-primary))]">
              You Undo the Auto {isCheckIn ? "Check in" : "Check out"} , please
              do action after selecting items
            </span>
            <button
              onClick={() => setUndoState("timer")}
              className="text-(--color-sunlit-gold) hover:opacity-80 transition-opacity"
            >
              <X size={16} />
            </button>
          </div>
      ...
      {showActionAlert && undoState === "timer" && (
        ...
        >
          <div className="flex justify-between items-center w-full">
            <span
              className={`text-sm font-bold ${isCheckIn ? "text-(--color-fresh-mint)" : "text-(--color-sky-blue)"}`}
            >
              10 tags auto {isCheckIn ? "Check in" : "Check Out"}
            </span>
            <button
              onClick={() => setShowActionAlert(false)}
              className={`${isCheckIn ? "text-(--color-fresh-mint)" : "text-(--color-sky-blue)"} hover:opacity-80 transition-opacity`}
            >
              <X size={16} />
            </button>
          </div>
