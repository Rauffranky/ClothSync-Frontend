...
import { Link, Search, Tag, Unlink, AlertCircle, Eye, History, Ban } from "lucide-react";
...
const TagsTable = () => {
    ...
    }, [searchValue, mappingFilter, categoryFilter, assetStatusFilter, statusFilter]);
    ...
    const columns = [
        ...
        },
        {
            ...
            sortable: true,
            render: (_, row) => (
                <div className={`flex items-center gap-1 text-xs font-bold ${row.mapping === 'Linked' ? 'text-(--color-ready)' : 'text-(--theme-text-muted)'}`}>
                    <Link size={14} />
                    {row.mapping}
                </div>
            ),
            ...
            render: (_, row) => {
                ...
                return (
                    <div className="min-w-0">
                        ...
                        <p className="m-0 mt-0.5 font-mono text-[11px] font-semibold text-(--color-sky-blue)">
                            {row.assetId}
                        </p>
                    </div>
                );
        ...
        },
        {
            ...
            sortable: true,
            render: (_, row) => {
                if (row.category === "—") return <span className="text-(--theme-text-muted)">—</span>;
                return (
                    <span className="inline-flex px-2 py-1 bg-(--theme-bg-soft) text-(--color-sky-blue) rounded-md text-xs font-semibold">
                        {row.category}
                    </span>
                );
            }
        },
        {
            ...
            sortable: true,
            render: (_, row) => {
                if (row.assetStatus === "—") return <span className="text-(--theme-text-muted)">—</span>;
        ...
        },
        {
            ...
            sortable: true,
            render: (_, row) => {
                const isActive = row.tagStatus === "Active";
                return (
                    <div className={`flex items-center gap-1 text-xs font-bold ${isActive ? "text-(--color-sky-blue)" : "text-(--theme-text-muted)"}`}>
                        <div className={`h-3 w-3 rounded-full flex items-center justify-center border border-current`}>
        ...
        },
        {
            ...
            render: (_, row) => {
                if (row.lastScanTime === "Never") {
                    return <span className="text-sm italic font-medium text-(--theme-text-muted)">Never</span>;
                }
                return (
                    <div className="min-w-0">
                        <p className="m-0 truncate font-mono text-[13px] font-bold text-(--theme-text-primary)">
                            {row.lastScanTime}
                        </p>
        ...
        },
        {
            ...
            render: (_, row) => (
                <ActionDropdown
                    items={[
                        { label: "View Details", icon: Eye, onClick: () => navigate(`/business/tags/${row.id}`) },
        ...
        <div className="">
            <div className="grid gap-2 px-4 py-4 lg:flex lg:flex-wrap lg:items-center">
                ...
                <div className="w-full lg:w-50">
                    <Dropdown
                        leftIcon={<AlertCircle size={14} className="text-(--theme-text-muted)" />}
                        onChange={(val) => {
