...
interface MobileScannerApi {
    ...
    @POST("../auth/refresh") suspend fun refresh(@Body body: RefreshRequest): ApiEnvelope<LoginData>
    ...
    @GET("sessions/{sessionId}") suspend fun session(@Path("sessionId") id: String): ApiEnvelope<SessionDto>
    @POST("sessions/{sessionId}/stop") suspend fun stop(@Path("sessionId") id: String): ApiEnvelope<SessionDto>
