...
@Composable private fun App(vm: ScannerViewModel = hiltViewModel()) {
    ...
    when {
        ...
        state.portal == "laundry" && state.batch == null -> BatchSelection(state, vm::selectBatch, vm::logout)
        else -> BulkScan(state, vm::start, vm::stop, vm::setManualAction, vm::logout)
    }
}
...
@Composable private fun ScannerSelection(state: com.clothsync.scanner.ui.UiState, select: (ScannerDto) -> Unit, logout: () -> Unit) { Page("Select Scanner") { Text("${state.portal.replaceFirstChar { it.uppercase() }} portal"); if (state.loading) CircularProgressIndicator(); state.error?.let { Text(it, color = MaterialTheme.colorScheme.error) }; state.scanners.forEach { scanner -> val selectable = scanner.actualUuid() != null; Card(onClick = { if (selectable) select(scanner) }, enabled = selectable, modifier = Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp)) { Text(scanner.displayName(), style = MaterialTheme.typography.titleMedium); Text("Scanner code: ${scanner.displayCode()}"); Text("Type: ${scanner.scannerType.orEmpty()} · Mode: ${scanner.scannerMode.orEmpty()}"); Text("Location: ${scanner.location ?: "—"}"); Text("Operator: ${scanner.assignedOperator?.fullName ?: "—"} · ${scanner.status.orEmpty()}"); if (!selectable) Text("Scanner UUID is missing", color = MaterialTheme.colorScheme.error) } } }; TextButton(logout) { Text("Logout") } } }
@Composable private fun BatchSelection(state: com.clothsync.scanner.ui.UiState, select: (BatchDto) -> Unit, logout: () -> Unit) { Page("Select Incoming Batch") { state.batches.forEach { batch -> Card(onClick = { select(batch) }, enabled = batch.actualId() != null, modifier = Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp)) { Text(batch.displayCode(), style = MaterialTheme.typography.titleMedium); Text(batch.displayBusiness()); Text("${batch.displayStatus()} · ${batch.displayTotal()} tags"); Text(batch.displayDispatchDate()) } } }; TextButton(logout) { Text("Logout") } } }
@Composable private fun BulkScan(state: com.clothsync.scanner.ui.UiState, start: () -> Unit, stop: () -> Unit, manual: (String) -> Unit, logout: () -> Unit) { Page("Bulk Scan") { Text(if (state.connected) "RFID Connected" else "RFID Disconnected"); Text("Scanner: ${state.scanner?.displayName().orEmpty()}"); Text("Mode: ${state.scanner?.scannerMode.orEmpty()}"); state.batch?.let { Text("Batch: ${it.displayCode()} · ${it.displayBusiness()}") }; if (state.scanner?.scannerMode.equals("manual", true)) { Row { FilterChip(state.manualAction == "check_in", { manual("check_in") }, { Text("Check In") }); Spacer(Modifier.width(8.dp)); FilterChip(state.manualAction == "check_out", { manual("check_out") }, { Text("Check Out") }) } }; Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Counter("Unique", state.unique); Counter("Processed", state.processed); Counter("Rejected", state.rejected); Counter("Pending", state.pending) }; Button(start, enabled = !state.scanning && !state.loading && state.scanner?.actualUuid() != null && (state.portal != "laundry" || state.batch?.actualId() != null), modifier = Modifier.fillMaxWidth()) { Text("Bulk Scan") }; Button(stop, enabled = state.scanning, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = MaterialTheme.colorScheme.error)) { Text("Stop Scan") }; Text(state.message); state.error?.let { Text(it, color = MaterialTheme.colorScheme.error) }; TextButton(logout) { Text("Logout") } } }
