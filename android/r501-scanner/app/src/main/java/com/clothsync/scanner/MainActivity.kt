...
@Composable private fun App(vm: ScannerViewModel = hiltViewModel()) {
    ...
    when {
        ...
        else -> BulkScan(state, vm::start, vm::stop, vm::clearData, vm::changeScanner, vm::setManualAction, vm::refreshScanner, vm::logout)
    }
}
...
@Composable private fun BulkScan(state: com.clothsync.scanner.ui.UiState, start: () -> Unit, stop: () -> Unit, clear: () -> Unit, changeScanner: () -> Unit, manual: (String) -> Unit, refresh: () -> Unit, logout: () -> Unit) { Page("Bulk Scan") { Text(if (state.connected) "RFID Connected" else "RFID Disconnected"); Text("Scanner: ${state.scanner?.displayName().orEmpty()}"); Text("Mode: ${state.scanner?.scannerMode.orEmpty()}"); Text("Location: ${state.scanner?.displayLocation() ?: "—"}"); if (BuildConfig.SCANNER_VARIANT == "fixed") Text("Fixed scanner is scanning automatically while active") else if (state.portal == "laundry") Text("Batches are identified automatically from scanned tags"); if (state.scanner?.scannerMode.equals("manual", true)) { Text("Action: ${when (state.manualAction) { "check_out" -> "Check Out"; "read_only" -> "Read only"; else -> "Check In" }}", style = MaterialTheme.typography.titleMedium); Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { FilterChip(!state.scanning && state.manualAction == "check_in", { if (!state.scanning) manual("check_in") }, { Text("Check In") }); FilterChip(!state.scanning && state.manualAction == "check_out", { if (!state.scanning) manual("check_out") }, { Text("Check Out") }); FilterChip(!state.scanning && state.manualAction == "read_only", { if (!state.scanning) manual("read_only") }, { Text("Read only") }) } }; Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Counter("Unique", state.unique); Counter("Processed", state.processed); Counter("Rejected", state.rejected) }; OutlinedButton(refresh, enabled = !state.loading && !state.scanning, modifier = Modifier.fillMaxWidth()) { Text(if (state.loading) "Refreshing…" else "Refresh Scanner Status") }; if (BuildConfig.SCANNER_VARIANT != "fixed") Button(start, enabled = !state.scanning && !state.loading && state.scanner?.actualUuid() != null && state.scanner.status.equals("active", true), modifier = Modifier.fillMaxWidth()) { Text("Bulk Scan") }; Button(stop, enabled = state.scanning, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = MaterialTheme.colorScheme.error)) { Text("Stop Scan") }; Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) { OutlinedButton(changeScanner, enabled = !state.loading, modifier = Modifier.weight(1f)) { Text("Back") }; OutlinedButton(clear, enabled = !state.scanning && !state.loading, modifier = Modifier.weight(1f)) { Text("Clear Data") } }; Text(state.message); if (state.results.isNotEmpty()) { Text("Scan results", style = MaterialTheme.typography.titleMedium); state.results.forEach { result -> Text("${result.epc} · ${result.statusLabel ?: result.status ?: if (result.accepted) "Accepted" else "Rejected"}${result.details?.let { " · $it" } ?: result.reason?.let { " · $it" } ?: ""}") } }; state.error?.let { Text(it, color = MaterialTheme.colorScheme.error) }; TextButton(logout) { Text("Logout") } } }
