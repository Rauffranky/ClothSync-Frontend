...
@Composable private fun BulkScan(state: com.clothsync.scanner.ui.UiState, start: () -> Unit, stop: () -> Unit, clear: () -> Unit, changeScanner: () -> Unit, manual: (String) -> Unit, logout: () -> Unit) { Page("Bulk Scan") { Text(if (state.connected) "RFID Connected" else "RFID Disconnected"); Text("Scanner: ${state.scanner?.displayName().orEmpty()}"); Text("Mode: ${state.scanner?.scannerMode.orEmpty()}"); Text("Location: ${state.scanner?.displayLocation() ?: "—"}"); state.batch?.let { Text("Batch: ${it.displayCode()} · ${it.displayBusiness()}") }; if (state.scanner?.scannerMode.equals("manual", true)) { Row { FilterChip(state.manualAction == "check_in", { manual("check_in") }, { Text("Check In") }); Spacer(Modifier.width(8.dp)); FilterChip(state.manualAction == "check_out", { manual("check_out") }, { Text("Check Out") }) } }; Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Counter("Unique", state.unique); Counter("Processed", state.processed); Counter("Rejected", state.rejected); Counter("Pending", state.pending) }; Button(start, enabled = !state.scanning && !state.loading && state.scanner?.actualUuid() != null && (state.portal != "laundry" || state.batch?.actualId() != null), modifier = Modifier.fillMaxWidth()) { Text("Bulk Scan") }; Button(stop, enabled = state.scanning, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = MaterialTheme.colorScheme.error)) { Text("Stop Scan") }; Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) { OutlinedButton(changeScanner, enabled = !state.loading, modifier = Modifier.weight(1f)) { Text("Back") }; OutlinedButton(clear, enabled = !state.scanning && !state.loading, modifier = Modifier.weight(1f)) { Text("Clear Data") } }; Text(state.message); state.error?.let { Text(it, color = MaterialTheme.colorScheme.error) }; TextButton(logout) { Text("Logout") } } }
